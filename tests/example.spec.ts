...
test('has title', async ({ page }, testInfo) => {
  ...
  const screenshotDir = path.join(__dirname, `../screenshots/${platform()}-${release()}/`, mode);
...
test('get started link', async ({ page }, testInfo) => {
  ...
  await installationHeading.screenshot({ 
    path: path.join(__dirname, `../screenshots/${platform()}-${release()}/`, mode, 'installation-heading.png') 
  });
});